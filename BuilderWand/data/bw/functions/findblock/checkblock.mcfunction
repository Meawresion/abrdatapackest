#a binary block check. Blocks with and without Blockstates get their own value on the scoreboard.
...
#get the Block onto a defined Position at the world border so that it can be cloned from there

#first reset the position before use, so the block from last use can't interfere.
fill -30000000 1 1610 -30000000 2 1610 air replace

#this happens for each player seperately and only in the 1 tick so it'll still be multiplayer compatible.
